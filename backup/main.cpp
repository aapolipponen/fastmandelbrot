    ...
    for (int y = startY; y < endY; y++) {
        for (int x = startX; x < endX; x++) {
            ...
            if (useSmoothColoring) {
                if (iter < MAX_ITER) {
                    double zn = sqrt(x2 + y2);
                    double nu = log(log(zn) / log(2)) / log(2);
                    color = (unsigned char)(255.0 * (iter + 1 - nu) / MAX_ITER);
                } else {
                    color = 0; // Black for points inside the Mandelbrot set
                }
            } else {
                color = iter % 256;
            }
    ...
    int handle(int event) override {
        ...
        case FL_MOUSEWHEEL:
            if (Fl::event_dy() != 0) {
                ...
                offsetX -= zoomFactor * (0.5 - mouseX) / zoom;
                offsetY -= zoomFactor * (0.5 - mouseY) / zoom;
